...
function patchPackageJson(cwd, log) {
  ...
  if (changed) {
    ...
    log('  ✓  package.json (added qa, qa:ui, qa:run scripts)');
  } else {
...
function printNextSteps(cwd) {
  ...
  const onboardingPath = path.join(PKG_ROOT, 'ONBOARDING.md');
  console.log(`
${'─'.repeat(72)}
  NEXT STEPS
${'─'.repeat(72)}

  1. (Optional) Copy the env template if your tests need credentials:
       ${copyCmd}  &&  edit .env
  ...
  3. Author tests via Claude Code (use the prompts in QAEnd2EndPromptFile.md
     or the "Copy prompt" panel in the UI) or hand-author per pages/README.md.
     Then click ▶ Run Tests in the UI to execute.

  Full docs: ${onboardingPath}
${'─'.repeat(72)}
`);
}
